...
console.log('\n1. Geofence Test:');
users.forEach(u => {
    const inside = (0, geofenceService_1.isPointInRegion)(u.location, mockRegion);
    console.log(`User ${u.id}: ${inside ? 'INSIDE' : 'OUTSIDE'} (Expected: ${u.id.includes('out') ? 'OUTSIDE' : 'INSIDE'})`);
});
// 2. PRIORITY TEST
console.log('\n2. Priority Test:');
const inRegionUsers = users.filter(u => (0, geofenceService_1.isPointInRegion)(u.location, mockRegion));
